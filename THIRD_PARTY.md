...
- `skills/diagnosing-bugs`: included with its upstream human-in-the-loop diagnostic loop.
...
- `tdd`: intentionally removed from the portable bundle at the learner's request; ordinary task verification remains part of `ai-native-task-tutor`, and a dedicated test-first skill can be added later if the team needs it.
